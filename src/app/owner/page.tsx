...
export default function HomePage() {
  ...
  // ✅ 메뉴 아이템 파싱 함수
  const parseMenuItems = (rawItems: any): MenuItem[] => {
    if (Array.isArray(rawItems)) {
      return rawItems.map((s: any) => {
        if (typeof s === "string") {
          return { name: s.trim(), category: "반찬" }
        }
        return { 
          name: s.name || String(s), 
          category: s.category || "반찬" 
        }
      })
    }
  ...
  const sortMenuItems = (items: MenuItem[]): MenuItem[] => {
    return [...items].sort((a, b) => {
      if (a.category === "메인" && b.category !== "메인") return -1
      if (a.category !== "메인" && b.category === "메인") return 1
      return 0
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            {filteredMenus.map((menu) => {
              // ✅ parseMenuItems로 데이터 파싱
              const parsedItems = parseMenuItems(menu.items)
              // ✅ sortMenuItems로 메인 메뉴 우선 정렬
              const items = sortMenuItems(parsedItems)

              return (
                <div key={menu.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-md hover:shadow-lg transition">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <button onClick={() => toggleBookmark(menu.restaurants?.name || "")} className="text-2xl hover:scale-110 transition">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {items.map((item, idx) => {
                      // ✅ 메인 메뉴 판별
                      const isMajor = item.category === "메인"
                      const icon = isMajor ? getMainDishIcon(item.name) : null
                      ...
                      if (isMajor && icon) {
                        ...
                        return (
                          <span
                            key={idx}
                            className="bg-gradient-to-r from-orange-100 to-red-100 border-2 border-orange-400 text-orange-900 px-4 py-2 rounded-full text-sm font-bold shadow-md hover:shadow-lg transition scale-105"
                          >
                            {icon} {item.name}
                          </span>
                        )
                      ...
                      return (
                        <span
                          key={idx}
                          className="bg-gray-50 border border-gray-300 text-gray-700 px-3 py-1.5 rounded-full text-xs font-medium hover:bg-gray-100 transition"
                        >
                          {item.name}
                        </span>
                      )
                  ...
                  <div className="flex gap-2">
                    <a
                      ...
                      target="_blank"
                      className="flex-1 text-center bg-yellow-400 text-yellow-900 py-2 rounded-xl text-sm font-bold hover:bg-yellow-500 transition shadow-sm"
                    >
                      카카오맵
                    </a>
                    <a
                      ...
                      target="_blank"
                      className="flex-1 text-center bg-green-500 text-white py-2 rounded-xl text-sm font-bold hover:bg-green-600 transition shadow-sm"
                    >
                      네이버지도
                    </a>
                  </div>
            ...
            {filteredNoMenu.map((r) => (
              <div key={r.id} className="bg-white rounded-2xl border border-dashed border-gray-300 p-5 shadow-sm opacity-70 hover:opacity-100 transition">
                <div className="flex justify-between items-start mb-3">
                  <div>
                  ...
                  <button onClick={() => toggleBookmark(r.name)} className="text-2xl hover:scale-110 transition">
                ...
                <div className="flex gap-2">
                  <a
                    href={`https://map.kakao.com/link/search/${encodeURIComponent(r.address)}`}
                    target="_blank"
                    className="flex-1 text-center bg-yellow-400 text-yellow-900 py-2 rounded-xl text-sm font-medium hover:bg-yellow-500 transition shadow-sm"
                  >
                    카카오맵
                  </a>
                  <a
                    href={`https://map.naver.com/v5/search/${encodeURIComponent(r.address)}`}
                    target="_blank"
                    className="flex-1 text-center bg-green-500 text-white py-2 rounded-xl text-sm font-medium hover:bg-green-600 transition shadow-sm"
                  >
                    네이버지도
                  </a>
                </div>
        ...
        {/* 광고 배너 */}
        <div className="mt-8 bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-5 border border-blue-100 shadow-md">
          ...
          <div className="flex gap-2">
            <a href="mailto:kbioin@naver.com" className="flex-1 text-center bg-blue-500 text-white py-2 rounded-xl text-sm font-medium hover:bg-blue-600 transition">이메일 문의</a>
            <a href="https://open.kakao.com/o/sV5Fw9mi" target="_blank" className="flex-1 text-center bg-yellow-400 text-yellow-900 py-2 rounded-xl text-sm font-medium hover:bg-yellow-500 transition">카카오톡 문의</a>
          </div>
        </div>

        {/* 사장님 배너 */}
        <div className="mt-4 bg-gradient-to-r from-orange-50 to-red-50 rounded-2xl p-5 border border-orange-200 shadow-md">
          ...
          <div className="flex gap-2">
            <a href="/owner/register" className="flex-1 text-center bg-orange-500 text-white py-2 rounded-xl text-sm font-bold hover:bg-orange-600 transition shadow-sm">가게 등록하기</a>
            <a href="/owner" className="flex-1 text-center bg-white text-orange-600 border border-orange-300 py-2 rounded-xl text-sm font-bold hover:bg-orange-50 transition">메뉴 올리기</a>
          </div>
        </div>
        ...
        <footer className="mt-8 mb-6 text-center text-xs text-gray-400">
          ...
          <p className="mt-1">
            <a href="mailto:kbioin@naver.com" className="underline hover:text-gray-600">문의</a>
            {" · "}
            <a href="https://open.kakao.com/o/sV5Fw9mi" target="_blank" className="underline hover:text-gray-600">카카오톡</a>
          </p>
        </footer>
      </div>
    </div>
