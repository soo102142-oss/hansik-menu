...
export default function OwnerPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("기존 메뉴 데이터:", existingData)

      // ✅ 2단계: 기존 데이터가 있으면 모두 삭제
      if (existingData && existingData.length > 0) {
        const idsToDelete = existingData.map((item: any) => item.id)
      ...
      if (insertError) {
        ...
        throw new Error("메뉴 등록 실패: " + insertError.message)
      }

      console.log("저장 성공:", insertedData)
      setLoading(false)
      setSuccess(true)
